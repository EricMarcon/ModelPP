...
cmp <- ??? ~ mySmooth(???, model = matern) + Intercept
...
#' And to plot these three with a nicer colour scheme:
#' 
#+results="hide",warning=FALSE,message=FALSE

csc <- colsc(lambda@data["median"],
             lambda@data["q0.025"],
             lambda@data["q0.975"]) ## Common colour scale from SpatialPixelsDataFrame

gmedian <- ggplot() + gg(lambda["median"]) + coord_equal() + csc
glower95 <- ggplot() + gg(lambda["q0.025"]) + coord_equal() + csc + theme(legend.position = "none")
gupper95 <- ggplot() + gg(lambda["q0.975"]) + coord_equal() + csc + theme(legend.position = "none")
...
#+results="hide",warning=FALSE,message=FALSE,echo=TRUE,eval=FALSE
...
#' and plot posteriors as per last practical:
