...
lambda_poish_df = as.data.frame(lambda_poish)                 # Pour chaque point de pix en ligne, on a la loi du lambda associé.
lambda_poish_df$intensity = lambda_poish$mean*(nb_carres_par_cote**2/25)          # Renormalisation pour avoir l'intensité, au lieu 
                                                                                  # du nombre de points par cellule de la grille.
...
# Critères WAIC et DIC

scores <- data.frame(
  dic=c(alt = fit_alt$waic$waic, spde = fit_spde$waic$waic, alt_spde = fit_alt_spde$waic$waic),
  waic=c(alt = fit_alt$dic$dic, spde = fit_spde$dic$dic, alt_spde = fit_alt_spde$dic$dic))
...
post.range = spde.posterior(fit_alt_spde, name="field", what="range"); plot(post.range)
post.matcorr = spde.posterior(fit_alt_spde, name="field",what="matern.correlation"); plot(post.matcorr)
```
